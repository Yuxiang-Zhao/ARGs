...
def resample_sequences_habitats(df, gene_tree, sample_tree, iterations, output_file, max_sample_size=30, shuffle_count=0):
    results = []
    ...
    for group in groups:
        ...
        for i in range(iterations):
            ...
            results.append({
                'Iteration': i + 1,
                'Group': group,
                'Gene Avg': calculate_normalized_connectivity(balanced_genes, habitats, gene_groups, shuffle_count, min_habitat_count)[2],
...
def main(habitat_file, gene_tree_file, sample_tree_file, iterations=999, output_file='paixu_cattle99_30.xlsx', max_sample_size=30, shuffle_count=0):
    ...
    resample_sequences_habitats(df, gene_tree, sample_tree, iterations, output_file, max_sample_size, shuffle_count)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--shuffle_count', type=int, default=0, help='Number of times to shuffle identical groups')

    args = parser.parse_args()

    main(args.habitat_file, args.gene_tree_file, args.sample_tree_file, iterations=args.iterations, output_file=args.output_file, max_sample_size=args.max_sample_size, shuffle_count=args.shuffle_count)
